...
export class ImportSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(el).setName('Zoteroとの接続').setDesc('Zotero 10以降を起動してください。').addButton(b => b.setButtonText('接続確認').onClick(async () => {
      b.setDisabled(true); try { await p.client().probe(); new Notice('接続できました。文献一覧も取得できています。'); } catch (e) { new Notice((e as Error).message, 15000); } finally { b.setDisabled(false); }
    })).addButton(b => b.setButtonText('接続案内').onClick(() => p.showSetup()));
    ...
    if (s.naming !== 'author-year') {
      ...
      new Setting(el).setName('AI CLI').addDropdown(d => d.addOption('codex', 'Codex').addOption('claude', 'Claude Code').addOption('opencode', 'OpenCode（実験的対応）').setValue(s.provider).onChange(async v => { s.provider = v as Provider; s.cliPath = ''; await p.saveSettings(); this.display(); }));
      ...
      new Setting(el).setName('モデル').setDesc('空欄は既定モデル。Codexは認証を再利用し、個人の設定・フック・MCPは読み込みません。').addText(t => t.setValue(s.model).onChange(async v => { s.model = v; await p.saveSettings(); }));
      ...
      new Setting(el).setName('AI命名できない場合').addDropdown(d => d.addOption('author-year', '著者名＋年で保存').addOption('ask', '名前を入力').setValue(s.fallback).onChange(async v => { s.fallback = v as 'ask' | 'author-year'; await p.saveSettings(); }));
    ...
    new Setting(details).setName('ローカルAPIポート').setDesc('通常は変更不要。この端末のZoteroにのみ接続します。').addText(t => t.setValue(String(s.port)).onChange(async v => { const n = Number(v); if (Number.isInteger(n) && n > 0 && n < 65536) { s.port = n; await p.saveSettings(); } }));
