...
### [⬇ インストール用ZIPをダウンロード（日本語版 0.3.0）](https://github.com/edoaki/zotero-paper-import/releases/download/0.3.0/zotero-paper-import-0.3.0.zip)

**上のリンクを押せば、必要なZIPを直接ダウンロードできます。GitHubへのログインは不要です。**

GitHubの緑色の「Code → Download ZIP」や「Source code (zip)」は開発用です。インストールには、必ず上のリンクの `zotero-paper-import-0.3.0.zip` を使ってください。

## Macでの入れ方

### 1. ZIPを展開する

Finderの「ダウンロード」で `zotero-paper-import-0.3.0.zip` をダブルクリックします。
...
4. **文献の親フォルダ**に、保管庫内のフォルダ名を入力します。例：`文献`。取り込み先と整理対象は自動で `文献/未整理` になります。まだないフォルダでも使えます。
...
1. 設定で「文献の親フォルダ」を選びます。`文献` なら `文献/未整理` の論文を `文献` 内へ分類します。未整理フォルダの名前は詳細設定で変更できます。
...
5. **「整理結果・分類先を変更」** で移動先と分類理由を確認できます。「分類先を変更」から別の分類先を選び、現在の論文フォルダを移動できます。
...
分類先を変更する際も、整理後に書き足したメモやPDFの編集内容を保持します。手動で分類先を指定した論文として保持し、元の分類フォルダは残します。過去の状態に巻き戻す「元に戻す」機能は廃止しました。

リンクを維持するため、Obsidianの「設定 → ファイルとリンク → 内部リンクを常に更新」をオンにしてください。整理自体はZoteroを起動していなくても使えます。

旧版の設定は自動で引き継ぎます。既存の論文を自動で移動することはありません。今後取り込む論文は、親フォルダ内の共通の未整理フォルダへ保存します。旧設定の場所も、取り込み済み照合とキー補完の対象として保持します。

## 表示されない・処理できない理由を確認する

取り込み画面の「取り込み済みで非表示の論文と理由」を開くと、Zoteroキーで一致したのか、DOI・arXiv IDで一致したのかを確認して既存ノートを開けます。整理画面の「整理できない項目と理由」には、ノートが複数ある、論文フォルダの外にある、手動分類で固定されているなどの理由と対処方法を表示します。処理に失敗した場合も、結果と対処方法を表示します。

## 既存ノートのZoteroキーを補完する

設定の「キーを補完」、またはコマンド「既存ノートのZoteroキーを補完」から開きます。Zoteroライブラリを選び、キーのないノートの「照合して補完」を押してください。バックグラウンドでDOI・arXiv IDを照合します。

候補が1件に決まる場合だけ、ノートのプロパティにZoteroキー・ライブラリ・登録状態・Zoteroリンクを追加します。ノート本文・PDF・既存のキーは上書きしません。識別子がない、候補がない、候補が複数ある場合は理由を表示します。Zotero側の登録内容を変更する処理ではありません。
...
**既存フォルダ全体は削除しないでください。** 設定の `data.json` やバックアップを残したまま更新できます。コピー後に「プラグインの再読み込み」を押すかObsidianを起動し直し、スイッチをオンにします。表示されるバージョンが **0.3.0** なら更新完了です。
